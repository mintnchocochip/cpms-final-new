...
const API_BASE_URL =
  process.env.API_BASE_URL || "http://localhost:3000/api/admin";
const AUTH_TOKEN =
  "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6IjY2Y2JmOGU3ZDg4NzdkMDEzODVjZDY3YSIsImVtYWlsSWQiOiJhZG1pbkB2aXQuYWMuaW4iLCJlbXBsb3llZUlkIjoiQURNSU4wMDEiLCJyb2xlIjoiYWRtaW4iLCJpYXQiOjE3NTgyMDY3NDEsImV4cCI6MTc1ODI5MzE0MX0.udp-Gjv3TdpbAWuYponH2kUf-l9N6BJbewNKoE1o3zs";
...
async function main() {
  ...
  for (const row of data) {
    ...
    console.log("Panel Employee IDs:", panelEmpIds);
    console.log("Resolved Faculty Records:", panelMemberDocs);
    ...
    if (!panel) {
      ...
      console.log("Panel found:", panel._id);
    }

    // Project lookup with logging
    const project = await Project.findOne({
      name: projectTitle,
    });
    if (!project) {
      // Optional: query backend to see if more info available, or log what is present
      console.error(
      ...
      try {
        ...
        console.log("Backend response for missing project:", res.data);
      } catch (err) {
        console.log(
      ...
      console.log("Project found:", project._id, "| Title:", project.name);
    }
