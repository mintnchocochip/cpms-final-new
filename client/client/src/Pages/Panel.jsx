...
const Panel = () => {
  ...
  // ✅ FIXED: Panel version of updated request status logic
  const getTeamRequestStatus = (team, reviewType) => {
    ...
    // ✅ FIXED: Check if any approved panel extension has expired
    const now = new Date();
    const currentIST = new Date(now.toLocaleString("en-US", {timeZone: "Asia/Kolkata"}));
    
    const hasExpiredApprovedRequest = team.students.some(student => {
      const requestKey = `${student.regNo}_${reviewType}`;
      const requestStatus = requestStatuses[requestKey]?.status;
      
      if (requestStatus === 'approved' && student.deadline && student.deadline[reviewType]) {
        try {
          const extensionDeadline = new Date(student.deadline[reviewType].to);
          const extensionDeadlineIST = new Date(extensionDeadline.toLocaleString("en-US", {timeZone: "Asia/Kolkata"}));
          const isExpired = currentIST > extensionDeadlineIST;
          console.log(`🔄 Panel extension check for ${student.name} ${reviewType}: expired = ${isExpired}`);
          return isExpired;
        } catch (e) {
          console.error(`Error checking panel extension expiry for ${student.name} ${reviewType}:`, e);
          return false;
        }
      }
      return false;
    ...
    // ✅ If approved panel extension has expired, treat as 'none' to allow new requests
    if (hasExpiredApprovedRequest) {
      console.log(`🔄 Approved panel extension for ${reviewType} has expired, allowing new request`);
      return 'none';
    }
  ...
  // ✅ FIXED: Panel version - same logic as Guide but with panel-specific logging
  const isTeamDeadlinePassed = (reviewType, teamId) => {
    ...
    // ✅ FIXED: Check individual overrides for reviews with APPROVED requests
    ...
    if (teamRequestStatus === 'approved') {
      ...
      if (studentsWithSpecificOverride.length > 0) {
        console.log(`👥 Found ${studentsWithSpecificOverride.length} students with APPROVED ${reviewType} panel individual overrides`);
