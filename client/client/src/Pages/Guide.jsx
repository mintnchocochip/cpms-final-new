...
const Guide = () => {
  ...
  // ✅ FIXED: Updated request status logic to allow multiple requests
  const getTeamRequestStatus = (team, reviewType) => {
    ...
    // ✅ FIXED: Check if any approved extension has expired
    const now = new Date();
    ...
    const hasExpiredApprovedRequest = team.students.some(student => {
      ...
      if (requestStatus === 'approved' && student.deadline && student.deadline[reviewType]) {
        try {
          ...
          return currentIST > extensionDeadlineIST;
        } catch (e) {
          return false;
        }
      }
    ...
    // ✅ If approved extension has expired, treat as 'none' to allow new requests
    if (hasExpiredApprovedRequest) {
      console.log(`🔄 Approved extension for ${reviewType} has expired, allowing new request`);
      return 'none';
    }
  ...
  // ✅ FIXED: Allow multiple request edits - check if current approved extension has expired
